...
select case (run_mode) ! run_mode is set outside, when solution is evaluated for closeness to a maxwellian...  
	...
	case (1) ! this is the non-linear perturbation mode: we need both the linear part and the non-linear 
	  ...
	  !!!!!!!!!!!!!
	  !!  MATH 582D, PROJECT 3  
	  !!  Estimating the effect of aliasing in evaluating the collision operator 
      ...
      !!!
      !!! Difference between Fourier and direct evaluation
      
      xxx0 = maxval(abs(fcol-fcol_scr))
      xxx1 = sum(abs(fcol-fcol_scr)*nodes_gwts)
      xxx2 = sum(abs(fcol-fcol_scr)**2*nodes_gwts)
	  print *,"Difference in evaluating collision directly and by FFT. Max Norm:", xxx0, "1-norm: ",xxx1, "2-norm", xxx2
      
	  !! Now we are addiing the dimensionless coefficient mol_diam^2*N_inf/(L_inf)^2 
	  coef_temp = (mol_diam/L_inf)**2*N_inf*dt
	  fcol = fcol*coef_temp
	  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 
	  !!! We wil now safe the value of the collision operator on hard drive
	  !!!
	  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	  call WriteCollOp_BGK0D (fcol,"FFBeval")
	  call WriteCollOp_BGK0D (fcol_scr,"DirEval")
	  !print *,"Linf err:", xxx0
      !print *,"L1 err:",xxx1
      !print *,"L2 err:",xxx2
      deallocate(fcol_scr)
